...
def da2triples(dialog_act):
    ...
    # assert triples != [] # save empty label in multiwoz
    return triples
                ...
                for i, _ in enumerate(tokens):
                    for span in spans:
                        if i == span[3]:
                            ...
                            tags.append("B-" + domain + '-' +intent.capitalize() + "+" + span[1].capitalize()) #B/I-domain-intent-slot
                            used_tags.append("B-" + span[1].capitalize())#B/I-slot
                            break
                        if span[3] < i <= span[4]:
                            domain,intent = span[0].split('-')
                            tags.append("I-" + domain + '-' +intent.capitalize() + "+" + span[1].capitalize()) 
